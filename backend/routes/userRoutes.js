...
router.get("/checkin", async (req, res) => {
  try {
    const id = global.currentUser?.uid ?? "NcVYtZR8FKesFUpiEzevcxwX6dL2";
    const userRef = await db.collection("user").doc(id);
    const response = await userRef.get();
    const user = response.data();
    const recordedDate = user.updated_at
      ? getDateDetails(user?.updated_at?._seconds * 1000)
      : null;
    const currentDate = getDateDetails();
    if (recordedDate?.nthDay != currentDate.nthDay)
      userRef
        .update({
          activeDays: FieldValue.increment(1),
          updatedDayNumber: currentDate.nthDay,
