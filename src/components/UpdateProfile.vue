<template>
  <div class="container px-4 py-10">
    <h2 class="base-page__heading-short">
      <BackButton />Update Profile
    </h2>
    <form class="mx-auto" style="max-width: 360px" encType="multipart/form-data" @submit.prevent="submitForm">
      <label class="auth-page__label" for="name">Email</label>
      <input class="auth-page__input" :value="store.state?.currentUserDetails?.email" readonly disabled />
